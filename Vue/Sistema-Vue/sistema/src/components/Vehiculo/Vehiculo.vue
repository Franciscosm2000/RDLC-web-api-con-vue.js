    ...
    <v-layout align-start>
        <v-flex>
            ...
            <v-toolbar flat color="white">
                <v-toolbar-title><v-icon>ballot</v-icon>Vehiculos</v-toolbar-title>
                    <v-divider
                    class="mx-2"
                    inset
                    vertical
                                ...
                                <v-row>
                                    ...
                                    >
                                        <v-chip
                                            ...
                                            >
                                            <span>ID :
                                             <label v-text="id_vehiculo"></label></span>
                                    ...
                                    <v-col
                                        ...
                                        md="6"
                                    >
                                        <v-text-field v-model="no_Chasis"
                                        label="Num chasis"
                                        required
                                        >
                                        </v-text-field>
                                    ...
                                    <v-col
                                        ...
                                        md="6"
                                    >
                                        <v-text-field v-model="Costo_dia"
                                        label="Costo por dia" type="number" min="1"
            ...
            >
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td>{{ props.item.id_vehiculo }}</td>
                    <td>{{ props.item.modelo }}</td>
                    <td>{{ props.item.placa }}</td>
                    <td>{{ props.item.no_Chasis }}</td>
                    <td>{{ props.item.Costo_dia }}</td>
                </template>
...
<script>
    ...
    export default {
        data(){
            return {
                ...
                headers: [
                    ...
                    { text: 'Id', value: 'id_vehiculo'},
                    { text: 'modelo', value: 'modelo' },
                    { text: 'placa', value: 'placa'},
                    { text: 'no_Chasis', value: 'no_Chasis'},
                    { text: 'motor', value: 'motor',sortable: false},
                    { text: 'color', value: 'color',sortable: false},
                    { text: 'Costoxdia', value: 'costo_dia',sortable: false},
        ...
        computed: {
            formTitle () {
                return this.editedIndex === -1 ? 'Nueva Vehiculo' : 'Actualizar Vehiculo'
            }
        },
        ...
        methods:{
                ...
                let me=this;
                    var aux = [];
                     ...
                     let configuracion= {headers : header};
                        axios.get('api/Modelo_Vehiculos/Select',configuracion).then(function(response){
                            aux = response.data;
                            aux.map(function(x){
                             me.modelos.push({text: x.modelo,value:x.id_modelo});
                             });
                             me.resultPantalla();
                        }).catch(err => {
            ...
            editItem (item) {
                this.id_vehiculo = item.id_vehiculo;
                this.id_modelo = item.id_modelo;
                this.placa = item.placa;
                this.no_Chasis = item.no_Chasis;
                this.motor = item.motor;
                this.color = item.color;
                this.Costo_dia = item.costo_dia;
                this.editedIndex=1;
                this.dialog = true
            },
            close () {
            ...
            guardar () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    axios.put('api/Vehiculos/Actualizar',{
                        'id_vehiculo':me.id_vehiculo,
                        'id_modelo':me.id_modelo,
                        'placa':me.placa,
                        'no_Chasis':me.no_Chasis,
                        'motor':me.motor,
                        'color':me.color,
                        'Costo_dia':me.Costo_dia,
                        'usuario':"123"
            ...
            validar(){
                ...
                if(!this.placa){
                    this.validaMensaje.push("La placa es un campo obligatorio.");
                }
                if(!this.no_Chasis){
                    this.validaMensaje.push("El no_Chasis es un campo obligatorio.");
                }
                 if(!this.motor){
                    this.validaMensaje.push("El motor es un campo obligatorio.");
                }
                if(!this.color){
                    this.validaMensaje.push("El color es un campo obligatorio.");
                }
                if(!this.Costo_dia && this.Costo_dia <= 0 ){
                    this.validaMensaje.push("El Costo_dia es un campo obligatorio y tiene que ser mayor a 0.");
                }
            ...
            activarDesactivarMostrar(accion,item){
                ...
                this.adIdTipo=item.id_vehiculo;
                if (accion==1){
                    this.adAccion=1;
                }
                else if (accion==2){
            ...
            activar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Vehiculos/Activar/'+this.adIdTipo+'/'+this.usuario,{},configuracion).then(function(response){
            ...
            desactivar(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                axios.put('api/Vehiculos/Desactivar/'+this.adIdTipo+'/'+this.usuario,{},configuracion).then(function(response){
                    me.adModalDesactivar=0;
                    me.adAccion=0;
                    me.adIdTipo="";
                    me.listar();
